...
class TestVaultIsolation(unittest.TestCase):
    ...
    def test_multi_user_vault_isolation(self):
        ...
        self.assertTrue(tx_b.empty or not any(tx_b["raw_description"].str.contains("ALPHA SALARY")))
        ...
        self.assertFalse(wrong_unlock)
        self.assertTrue("Incorrect master passphrase" in err_msg or "denied" in err_msg)

        # 7. Test User B's passphrase FAILS on Vault A
        wrong_unlock_2, _, _ = VaultManager.unlock_vault(vault_a_id, "PassphraseBeta456!")
        self.assertFalse(wrong_unlock_2)
